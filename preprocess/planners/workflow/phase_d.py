"""
Phase D — slide through the timeline and mark which ICD diagnoses are groundable.
(Formerly diagnosis_scanner.py)
"""
...
def phase_d(client, record, verbose=True, model=None, window_size=100, step_size=50):
    """
...
"""
        try:
            ...
            if verbose:
                print(f"  [D] JSON Decode Error at window {start_idx}-{end_idx}: {e}")
        except Exception as e:
            if verbose:
                print(f"  [D] Fatal Error at window {start_idx}-{end_idx}: {e}")
            raise e
